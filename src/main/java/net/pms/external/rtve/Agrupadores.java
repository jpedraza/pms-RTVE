...
public class Agrupadores extends VirtualFolder {

    private static final long REFRESH_TIME = 86400000; // one day
    private static final Logger logger = LoggerFactory.getLogger(Agrupadores.class);
    private AgrupadorDTO agrupadorDTO;
    private List<ProgramDTO> programsDTO;
    private int nextPage;

    public Agrupadores(AgrupadorDTO agrupadorDTO) {
        super(agrupadorDTO.getName(), "");
        this.agrupadorDTO = agrupadorDTO;
    ...
    public void doRefreshChildren() {
        try {
            ...
            logger.info("RTVE: Refreshing programs of " + agrupadorDTO.getName());
            discoverChildren();
        } catch (Exception e) {
    ...
    private void getAllChildrens(AgrupadorDTO agrupador) {
        ...
        if (response != null && response.getPage().getTotal().intValue() > 0) {
            for (AgrupadorDTO childDTO : response.getPage().getItems().getAgrupadores()) {
                addChild(new Agrupadores(childDTO));
            }
        }
    }
}
